...
<html lang="en" class="no-js">
  ...
  <body>
	  ...
	  <nav class="navbar navbar-expand-lg navbar-dark bg-white fixed-top" id="mainNav">
		<div class="container">
		  ...
		  <div class="collapse navbar-collapse" id="navbarResponsive">
			...
			<li class="nav-item">
				<a class="nav-link js-scroll-trigger active" href="dashboardResepsionis">Dashboard</a>
            </li>
            <li class="nav-item">
    ...
    <section class="wt-section" id="dashboard" style="margin-left:5%; margin-right:5%">
    <h1 style="text-align:center">Daftar Check-in</h1>
    <a href="tambahcheckin"><button type="button" class="btn btn-success" style="float:right">Tambah</button></a>
    <br>
    <br>
        <table class="table table-striped table-bordered" id="checkin">
        <thead class="thead-dark">
            <tr>
            <th scope="col">Nama</th>
            <th scope="col">Kode Check-in</th>
        ...
        <tbody>
            @if (count($datacheckin) > 0)
            @foreach ($datacheckin as $data)
                    <tr>
                        <?php
                        $nama = DB::table('customer')->where('Kodecust',$data->Kodecust)->value('nama');
                        ?>
                        <td>{{$nama}}</td>
                        <td>{{$data->Kodecin}}</td>
                        <td>{{$data->Tglcin}}</td>
                        <td>{{$data->Tglcout}}</td>
                        <td>{{$data->NoKamar}}</td>
                        <td><a href="{{url("detailcheckin/".$data->Kodecin."")}}"><button type="button" class="btn btn-primary">Detail</button></a></td>
                    </tr>
                @endforeach
            @endif
